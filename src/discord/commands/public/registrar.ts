...
createResponder({
  ...
  types: ["Button" as any], cache: "cached",
  async run(interaction) {
    try {
      const msg = interaction.message as any;
      const container = msg.components?.[0];
      const textDisplay = container?.components?.find((c: any) => c.type === 10);
...
async function registrarMenu<R>(): Promise<R> {
    ...
    const container = createContainer("#2b2d31", 
        new TextDisplayBuilder().setContent(text),
        new ActionRowBuilder<ButtonBuilder>().addComponents(
            new ButtonBuilder({ customId: "btn-abrir-registro", label: "📝 Registrar", style: ButtonStyle.Success })
        )
    );
    return ({ flags: ["IsComponentsV2"], components: [container] } satisfies InteractionReplyOptions) as R;
}
...
createResponder({
    ...
    async run(interaction) {
        ...
        const inputNome = new ActionRowBuilder<TextInputBuilder>().addComponents(
            new TextInputBuilder({ customId: "input-nome", label: "NOME", style: TextInputStyle.Short, required: true, placeholder: "Ex: João" })
        );
        const inputRecrutador = new ActionRowBuilder<TextInputBuilder>().addComponents(
            new TextInputBuilder({ customId: "input-recrutador", label: "RECRUTADOR", style: TextInputStyle.Short, required: true, placeholder: "Ex: Fulano da Silva" })
        );
        const inputId = new ActionRowBuilder<TextInputBuilder>().addComponents(
            new TextInputBuilder({ customId: "input-id", label: "ID", style: TextInputStyle.Short, required: true, placeholder: "Ex: 12345" })
        );
        const inputTelefone = new ActionRowBuilder<TextInputBuilder>().addComponents(
            new TextInputBuilder({ customId: "input-telefone", label: "TELEFONE", style: TextInputStyle.Short, required: true, placeholder: "Ex: 000-000" })
        );
...
createResponder({
    ...
    async run(interaction) {
        ...
        try {
            const channel = await (interaction.client as any).channels.fetch(configCentral.canalRegistro);
            if ((channel as any)?.isTextBased?.()) {
                ...
                const sent = await (channel as any).send({ components: [panel], flags: ["IsComponentsV2"] });
...
createResponder({
    ...
    types: ["SelectMenu" as any], cache: "cached",
    async run(interaction) {
        try {
            ...
            const modal = new ModalBuilder({ customId: "modal-registro", title: "Formulário de Registro" });
            const inputNome = new ActionRowBuilder<TextInputBuilder>().addComponents(
                new TextInputBuilder({ customId: "input-nome", label: "NOME", style: TextInputStyle.Short, required: true, placeholder: "Ex: João Silva" })
            );
            const inputRecrutador = new ActionRowBuilder<TextInputBuilder>().addComponents(
                new TextInputBuilder({ customId: "input-recrutador", label: "RECRUTADOR", style: TextInputStyle.Short, required: true, placeholder: "Ex: Fulano da Silva" })
            );
            ...
            try {
                await (interaction as any).reply({ content: "Não foi possível processar a seleção do recrutador. Tente novamente.", ephemeral: true });
            } catch {}
        }
    }
});
